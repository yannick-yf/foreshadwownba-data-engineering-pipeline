...
def player_salary_data_acquisition(config_path: Text) -> pd.DataFrame:
    ...
    logger = get_logger(
        'PLAYER_SALARIES_DATA_ACQUISITION', 
    ...
    for k in range(2, page_total + 1):
        url = 'http://www.espn.com/nba/salaries/_/year/' + str(season) + '/page/' + str(k)
    ...
    for i in range(len(player_salary_df)):
        ...
        player_salary_df.loc[i, 'salary'] = re.sub('\$', '', player_salary_df['SALARY'][i])
        player_salary_df.loc[i, 'salary'] = re.sub(',', '', player_salary_df['salary'][i])
