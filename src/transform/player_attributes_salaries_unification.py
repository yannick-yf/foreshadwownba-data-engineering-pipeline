...
def player_attributes_salaries_unification(config_path: Text) -> pd.DataFrame:
    ...
    logger = get_logger(
        "PLAYER_ATTRIBUTES_SALARIES_UNIFICATION", log_level=config["base"]["log_level"]
    )
    ...
    players_attributes = pd.concat(
        map(
            pd.read_csv,
            glob.glob(
                os.path.join(
                    "pipeline_output/player_attributes/", "player_attributes_*.csv"
                )
            ),
        )
    )
    players_salary = pd.concat(
        map(
            pd.read_csv,
            glob.glob(
                os.path.join("pipeline_output/player_salary/", "player_salary_*.csv")
            ),
        )
    )

    # Name cleaning from both player attributes and player salary
    players_attributes["Name"] = players_attributes["Name"].str.replace(
        "'", "", regex=True
    )
    players_attributes["Name"] = players_attributes["Name"].str.replace(
        "-", "", regex=True
    ...
    # Birth date as date
    players_attributes["BirthDate"] = pd.to_datetime(
        players_attributes["BirthDate"], format="%B %d, %Y"
    )
    ...
    players_attributes["Age"] = (
        pd.to_datetime(players_attributes["game_date"])
        - pd.to_datetime(players_attributes["BirthDate"])
    ).dt.days
    ...
    players_attributes["Experience"] = pd.to_numeric(
        np.where(
            players_attributes["Experience"] == "R", 0, players_attributes["Experience"]
        )
    )
    ...
    players_attributes[["foot", "inch"]] = players_attributes["Ht"].str.split(
        "-", expand=True
    )
    players_attributes["cm_size"] = (
        pd.to_numeric(players_attributes["foot"]) * 30.48
        + pd.to_numeric(players_attributes["inch"]) * 2.54
    )
    ...
    name_and_path_file = data_path + file_name + ".csv"
...
if __name__ == "__main__":
    ...
    player_attributes_salaries_unification(config_path=args.config)
